...
/*
    if(i_pPort == P5){
        switch(i_u16Bit){
        case BIT5: o_u32AnalogChannel = ADC14_MCTLN_INCH_0;
        break;
        case BIT4: o_u32AnalogChannel = ADC14_MCTLN_INCH_1;
        break;
        case BIT3: o_u32AnalogChannel = ADC14_MCTLN_INCH_2;
        break;
        case BIT2: o_u32AnalogChannel = ADC14_MCTLN_INCH_3;
        break;
        case BIT1: o_u32AnalogChannel = ADC14_MCTLN_INCH_4;
        break;
        case BIT0: o_u32AnalogChannel = ADC14_MCTLN_INCH_5;
        break;
...
bool ADC::quick_setup(bool i_bInterruptEnable){
    ...
    for(int i = 0; i<m_vpPorts.size(); i ++){
    ...
    //CTL0 Config
    ADC14->CTL0 = ADC14_CTL0_SHP | ADC14_CTL0_SHT0_0 | ADC14_CTL0_ON |ADC14_CTL0_MSC;
    if(m_vpPorts.size()>1) ADC14->CTL0 |= ADC14_CTL0_CONSEQ_3;
...
bool ADC::setup(bool i_bInterruptEnable){
    if(m_configured){
        ADC14->CTL0 = m_u32CTL0 &~ ADC14_CTL0_ENC;
...
int16_t ADC::read(DIO_PORT_Even_Interruptable_Type * i_pPort, uint16_t i_u16pBit){
    ...
    for(uint8_t i = 0; i<32 ; i++){
        if(ADC14->MCTL[i] == l_u32AnalogChannel){
            result = ADC14->MEM[i];
            result >>= 2;
            if(result & 0x2000){
                result |= 0xC000;
